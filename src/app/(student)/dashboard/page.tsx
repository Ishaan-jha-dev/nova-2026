...
// Reusable starburst — same as PageWrapper so background is consistent everywhere
function StarburstCorner({ opacity = 0.5 }: { opacity?: number }) {
  ...
  return (
    <svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg"
      ...
      <g fill="none" stroke="#E8A020">
        {arms.map(deg => {
          ...
          return (
            <g key={deg}>
              ...
              <rect x={x2 - 4} y={y2 - 4} width="8" height="8" transform={`rotate(45 ${x2} ${y2})`} fill="#E8A020" fillOpacity="0.5" strokeWidth="0" />
              {[-25, 25].map(offset => {
                const forkRad = ((deg + offset) * Math.PI) / 180
                const mx = 150 + Math.cos(rad) * 75
                const my = 150 + Math.sin(rad) * 75
                return <line key={offset} x1={mx} y1={my} x2={mx + Math.cos(forkRad) * 30} y2={my + Math.sin(forkRad) * 30} strokeWidth="0.6" stroke="#E8A020" opacity="0.45" />
              })}
            </g>
          )
        })}
        {subArms.map(deg => {
          const rad = (deg * Math.PI) / 180
          return <line key={deg} x1={150 + Math.cos(rad) * 20} y1={150 + Math.sin(rad) * 20} x2={150 + Math.cos(rad) * 80} y2={150 + Math.sin(rad) * 80} strokeWidth="0.6" stroke="#E8A020" opacity="0.35" />
        })}
        ...
        {[[60,80],[90,40],[200,60],[240,90],[70,200],[50,240],[210,210],[250,250],[130,30],[170,270],[30,130],[270,170]].map(([cx, cy], i) => (
          <rect key={i} x={cx - 3} y={cy - 3} width="6" height="6" transform={`rotate(45 ${cx} ${cy})`} fill="#E8A020" fillOpacity="0.4" strokeWidth="0" />
        ))}
      </g>
    </svg>
  )
  ...
  return (
    ...
    >
      {/* ── Shared starfield dots (same as PageWrapper) ─── */}
