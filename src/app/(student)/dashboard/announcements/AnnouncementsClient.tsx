          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-3 mb-2">
                <h3 className="text-lg font-semibold text-nova-text group-hover:text-nova-primary transition-colors break-words min-w-0">{a.title}</h3>
                {a.target_audience && (
                  <span className="shrink-0 px-2.5 py-1 rounded-full bg-nova-primary/10 text-nova-primary border border-nova-primary/20 text-[10px] font-bold tracking-wider uppercase max-w-[100px] sm:max-w-xs truncate">
                    {a.target_audience}
                  </span>
                )}
              </div>
              <p className="text-sm text-nova-muted whitespace-pre-wrap leading-relaxed mb-4 line-clamp-3 break-words">{a.body}</p>
              <div className="flex flex-wrap items-center gap-4 text-xs text-nova-text-dim mt-auto pt-4 border-t border-white/5">
                <span className="flex items-center gap-1.5">
                  <Calendar size={13} className="text-nova-primary/70 shrink-0" />
                  <span className="truncate">{formatIST(a.created_at, 'MMM d, yyyy • h:mm a')}</span>
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-4 h-4 shrink-0 rounded-full bg-nova-primary/20 flex items-center justify-center text-[9px] font-bold text-nova-primary">
